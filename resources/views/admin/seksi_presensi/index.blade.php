...
@section('content')
    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Data Seksi Presensi</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ url('/admin') }}">Master Data</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Seksi Presensi</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="mb-2">
                        <h5 class="mb-0">Data Seksi Presensi</h5>
                    ...
                    <div class="table-responsive">
                        <table id="zero_config" class="table border table-bordered text-nowrap align-middle">
                            ...
                            <tbody>
                                @foreach ($data_seksi_presensi as $seksi_presensi)
                                    <!-- start row -->
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $seksi_presensi->guru->nama }}</td>
    ...
    data-bs-target="#modalHapus{{ $seksi_presensi->id }}">
    Hapus
</button>


                                            <!-- Modal Konfirmasi -->
                                            <div class="modal fade" id="modalHapus{{ $seksi_presensi->id }}" tabindex="-1"
                                                aria-labelledby="modalHapusLabel{{ $seksi_presensi->id }}" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-danger text-white">
                                                            <h5 class="modal-title"
                                                                id="modalHapusLabel{{ $seksi_presensi->id }}">Konfirmasi Hapus
                                                            </h5>
                                                            <button type="button" class="btn-close btn-close-white"
                                                                data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            Apakah Anda yakin ingin menghapus seksi presensi
                                                            <b>{{ $seksi_presensi->guru->nama }}</b>?
                                                        </div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary"
                                                                data-bs-dismiss="modal">Batal</button>

                                                            <form action="{{ url('/admin/delete_seksi_presensi/' . $seksi_presensi->id) }}"
        ...
        <div class="modal-content">
            <div class="modal-header d-flex align-items-center">
                <h4 class="modal-title" id="myModalLabel">
                    Tambah Seksi Presensi
                </h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ url('/admin/store_seksi_presensi') }}" method="post">
                @csrf
                <div class="modal-body">
